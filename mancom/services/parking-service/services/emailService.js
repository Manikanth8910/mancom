...
// Helper to load and compile template
...
class EmailService {
  ...
  /**
   * Send parking violation notification
   * @param {string} email - User's email
   * @param {string} name - User's name
   * @param {string} vehicleNumber - Vehicle number
   */
  async sendParkingViolationEmail(email, name, vehicleNumber, issueType, messageText) {
    try {
      const transporter = createTransporter();
      ...
      const mailOptions = {
        from: process.env.ADMIN_EMAIL,
        to: email,
        subject: `Parking Violation Alert: ${vehicleNumber}`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #dc2626;">Parking Violation Notice</h2>
            <p>Dear ${name},</p>
            <p>Your vehicle <strong>${vehicleNumber}</strong> has been flagged for a rule violation.</p>
            ...
            <p>Proper parking helps maintain order and safety in our campus parking areas.</p>
            <p>Thank you for your cooperation.</p>

            <br>
            <p>Best regards,<br>VNR Security Team</p>
          </div>
        `
      };

      const result = await transporter.sendMail(mailOptions);
      console.log('Parking violation email sent successfully:', result.messageId);
      return { success: true, messageId: result.messageId };
    } catch (error) {
      console.error('Error sending parking violation email:', error);
      return { success: false, error: error.message };
    }
  }
}
